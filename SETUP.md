...
### 승인된 도메인 등록 — 이 앱에서는 **필수가 아닙니다**

승인된 도메인 목록이 막는 것은 **소셜 로그인 팝업/리다이렉트(Google 등)와
이메일 링크 로그인**입니다. 이 앱이 쓰는 두 가지는 해당하지 않습니다:

- **Firestore** (참가자 앱 전부) — 승인된 도메인과 무관
- **이메일/비밀번호 로그인** (관리자) — 리다이렉트가 없어 도메인 검사를 안 함

즉 새 도메인에 올려도 그냥 동작합니다. 다만 나중에 Google 로그인을 추가하거나
비밀번호 재설정 메일을 쓰게 되면 그때는 등록해야 하므로, 미리 넣어두면 편합니다.
...
| 관리자 로그인 시 `auth/unauthorized-domain` | 승인된 도메인에 배포 주소 추가 (6단계). 이메일/비밀번호 로그인만 쓴다면 이 오류는 잘 안 납니다 |
